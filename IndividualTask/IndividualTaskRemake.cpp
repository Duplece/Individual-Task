...
void loading(vector<Parametr>& parametri, vector<Clients>& consumer, vector <Service>& servicee, vector <PodklService>& provideservice);

// Функция создания отчёта.
void report(vector<Parametr>& parametri, vector<Clients>& consumer, vector <Service>& servicee, vector <PodklService>& provideservice);
...
void loading(vector<Parametr>& parametri, vector<Clients>& consumer, vector <Service>& servicee, vector <PodklService>& provideservice) {
	...
	else
	{
		...
		while (getline(client, line)) // Получаем строку на вход
		{
			...
			temp >> clien.Surname >> clien.Name >> clien.Otchestvo >> clien.Phone >> clien.Nacahlo >> clien.Konez >> clien.Dolg >> clien.Vdolg; // Записываем строку
		...
		while (getline(podklservice, line))
		{
			...
			temp2 >> provserv.Phone >> provserv.UseKod >> provserv.DataUse.Day >> provserv.DataUse.Mounth >> provserv.DataUse.Year >> provserv.VremUse >> provserv.HowVremUSe;
...
void report(vector<Parametr>& parametri, vector<Clients>& consumer, vector <Service>& servicee, vector <PodklService>& provideservice)
{
	...
	// непростое условие, в котором можно запутаться, проверяет соответствие параметра диапозонау
		...
		for (int i = 0; i < servicee.size(); i++)
		{
			...
			else
			{
				...
				for (int k = 0; k < provideservice.size(); k++)
					if (servicee[i].Kod == provideservice[k].UseKod)
						if (provideservice[i].DataUse.Mounth == mes)
	...
	{
		otch << "Список услуг, предоставленных в " << mes << " месяце, которые попадают в задный диапозон." << endl;
		for (int i = 0; i < report.size(); i++)
		{
			otch << report[i].usluga << endl;
		}
	}
	cout << "Отчёт создан" << endl << "Выход";
	otch.close();
}
